class CatapultVehicle extends GGSVehicle
	placeable;
...
simulated event PostBeginPlay()
{
	...
	for( i = 0; i < mNumberOfSeats; i++ )
	{
		...
		// Add custom seats
		mPassengerSeats[i].VehiclePassengerSeat = Spawn( class'CatapultSeat' );
...
function InitCatapult(GGGoat Catapult_owner)
{
	local RB_BodyInstance bodyInst;

	Catapultowner=Catapult_owner;
	...
	// Create beam effect
    mBeam=Spawn(class'CatapultBeam');
    mBeam.SetBase(self);
    mBeam.SetHidden(true);
    // Create inventory
    mHold = Spawn( class'CatapultHold', self );
...
function OpenCloseCatapult()
{
	...
	if(isBeamActive)
	{
		...
		isBeamActive=false;
		mBeam.SetHidden(true);
		SetTimer(mOpenCloseAnimTime, false, NameOf( CatapultClosed ));
	}
	else
	{
		isOpening=true;
		SetTimer(mOpenCloseAnimTime, false, NameOf( CatapultOpened ));
	}
	ManageAnim();
}

function CatapultOpened()
{
	isOpening=false;
	isBeamActive=true;
...
function BeamAttractionEffect(float deltaTime)
{
	...
	else
	{
		foreach mBeamedActors(attrActor)
		{
			...
			if(attrPawn != none)
			{
				if(attrPawn == CatapultOwner && (attrPawn.Physics == PHYS_Falling || attrPawn.Physics == PHYS_Flying))
				{
...
function AddEasterEgg(optional int eeID)
{
	...
	{
		case 1:
			easterEggNpc=Spawn(class'CatapultNpcSheep', self,,,,, true);
			break;
		case 2:
		...
		case 3:
			...
			break;
		case 4:
			easterEggNpc=Spawn(class'CatapultNpcWhale', self,,,,, true);
			break;
		case 5:
			easterEggNpc=Spawn(class'CatapultNpcTRex', self,,,,, true);
			break;
		case 6:
			easterEggNpc=Spawn(class'CatapultNpcCow', self,,,,, true);
			break;
		case 7:
			easterEggNpc=Spawn(class'GGNpc', self,,,,, true);
			...
			break;
		case 8:
			easterEggKact=Spawn(class'CatapultAsteroid', self,,,,, true);
			break;
		case 9:
...
DefaultProperties
{
	...
	Begin Object name=SVehicleMesh
		SkeletalMesh=SkeletalMesh'Catapult.Mesh.Catapult_Skele_01'
		PhysicsAsset=PhysicsAsset'Catapult.mesh.Catapult_Skele_01_Physics'//This have a crappy collision box
		Animations=MyMeshSequence
		AnimSets(0)=AnimSet'Catapult.Anim.Catapult_Anim_01'
